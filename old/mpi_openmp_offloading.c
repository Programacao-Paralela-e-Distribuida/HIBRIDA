...
{
  int i, ranque, numprocs, provided;
...
#pragma omp target teams distribute parallel for default(none) firstprivate(num_steps, step, ranque, numprocs) reduction(+ : sum) map(tofrom : sum) private(x) device(0)
  for (i = ranque; i < num_steps; i += numprocs) { // Saltos de acordo com o número de processos
    x = (i + 0.5) * step;
    sum += 4.0 / (1.0 + x * x);
  }
